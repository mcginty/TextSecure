...
public class EmojiView extends LinearLayout {
  ...
  private void init() {
    ...
    for (int i = 0; ; i++) {
      if (i >= Emoji.data.length) {
        ...
        tabs.setIndicatorHeight(Emoji.getInstance(getContext()).scale(2.0F));
        tabs.setUnderlineHeight(Emoji.getInstance(getContext()).scale(2.0F));
        ...
        localLinearLayout.addView(localImageView, new LinearLayout.LayoutParams(Emoji.getInstance(getContext()).scale(61.0F), -1));
        this.recentsWrap = new FrameLayout(getContext());
        this.recentsWrap.addView(this.views.get(0));
        TextView localTextView = new TextView(getContext());
        localTextView.setText("No recent emoji"); // XXX stringify
        localTextView.setTextSize(18.0F);
        localTextView.setTextColor(-7829368);
        localTextView.setGravity(17);
        this.recentsWrap.addView(localTextView);
        this.views.get(0).setEmptyView(localTextView);
        addView(localLinearLayout, new LinearLayout.LayoutParams(-1, Emoji.getInstance(getContext()).scale(48.0F)));
        addView(this.pager);
        loadRecents();
        return;
      }
      GridView localGridView = new GridView(getContext());
      localGridView.setColumnWidth(Emoji.getInstance(getContext()).scale(45.0F));
  ...
  private class EmojiGridAdapter extends BaseAdapter {
    ...
    public View getView(int paramInt, View paramView, ViewGroup paramViewGroup) {
      ...
      localObject.setImageDrawable(Emoji.getInstance(getContext()).getEmojiBigDrawable(this.data[paramInt]));
