...
public class ApplicationPreferencesFragment extends PreferenceFragment
    ...
    public void onDestroy() {
        MemoryCleaner.clean((MasterSecret) getActivity().getIntent().getParcelableExtra("master_secret"));
        super.onDestroy();
    }
    ...
    public void onActivityResult(int reqCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (reqCode) {
                case PICK_IDENTITY_CONTACT:
                    handleIdentitySelection(data);
                    break;
                case ENABLE_PASSPHRASE_ACTIVITY:
                    getActivity().finish();
                    break;
            }
        }
    ...
    private String buildOutgoingSmsDescription() {
        ...
        if (dataFallback) {
            ...
            if (dataFallbackAsk)
                builder.append(" ").append(getString(R.string.preferences__sms_fallback_push_users_ask));
        }
        if (nonData) {
