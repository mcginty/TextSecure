...
public class PushDownloader {
  ...
  private void retrievePart(final MasterSecret masterSecret, final PduPart part, final long messageId, final long partId) {
    ...
    try {
      ...
      PushServiceSocket.DownloadProgressListener listener = new PushServiceSocket.DownloadProgressListener() {
        @Override
        public void onProgressUpdate(long downloadedBytes, long totalBytes) {
          Log.i("PushDownloader", "got onProgressUpdate for " + messageId + ": " + downloadedBytes + "/" + totalBytes);
  ...
  private File downloadAttachment(String relay, long contentLocation, PushServiceSocket.DownloadProgressListener listener)
      throws IOException
  {
